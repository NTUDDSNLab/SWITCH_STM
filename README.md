...
1. **_CI**: Utilizes Contention Intensity to calculate the current conflict level. If the conflicts exceed a threshold, the scheduler will temporarily pause switching until the Contention Intensity drops below the threshold.
2. **_CI_backoff**: In addition to pausing switching when conflicts are too high, this configuration also involves a random backoff period when an abort occurs. The maximum duration of this random backoff is proportional to the number of threads.
